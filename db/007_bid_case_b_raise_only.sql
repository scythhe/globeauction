-- 007_bid_case_b_raise_only.sql
-- BACKEND_SPEC.md §5.3 Case B is "a bidder increasing their own ceiling" —
-- raise-only. The old Case B branch only re-checked the generic
-- current_price + increment floor (computed in the shared validation step
-- above), which Case B itself never moves current_price past. That let a
-- leading bidder call place_bid again with a *lower* max_amount than their
-- existing one, as long as it still cleared that unrelated floor, silently
-- dropping their own ceiling. Found by test, not a pentest.
--
-- Same 5-arg signature as 004_bid_ip_tracking.sql, so CREATE OR REPLACE
-- is enough — no DROP needed.
