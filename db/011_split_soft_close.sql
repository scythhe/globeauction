-- 011_split_soft_close.sql
-- soft_close_window did two jobs with one value: "how close to the end
-- triggers an extension" and "how much time gets added." Decided with the
-- business (grounded against how Copart's own bonus-time mechanic
-- actually behaves, not assumed): those should be different magnitudes
-- for an async, week-long single-lot auction — a 5-minute reaction
-- window, extended by a short handful of seconds each time, not another
-- 5 minutes. Repeatable on every bid that lands in the window, same as
-- before — a one-time-only extension would be a bigger fairness change
-- than what was actually asked for.

alter table auctions rename column soft_close_window to soft_close_trigger;
alter table auctions alter column soft_close_trigger set default '5 minutes';

alter table auctions add column soft_close_extension interval not null default '15 seconds';
...
begin
  ...
  -- Soft close: a bid landing within soft_close_trigger of the deadline
  -- pushes ends_at out by soft_close_extension — a short, repeatable
  -- nudge (not another full trigger-window's worth of time), same
  -- mechanism as before this migration, just no longer the same value
  -- for both halves.
  --
  -- GREATEST, not a bare assignment: when trigger and extension were the
  -- same value, "time remaining < window" always meant "now + window is
  -- later than the current deadline," so overwriting ends_at unconditionally
  -- never moved it backward. Now that extension (15s) is smaller than
  -- trigger (5min), a bid landing early in that window — say 4 minutes
  -- out — has more time left than a bare `now + extension` would leave,
  -- and a naive assignment would have shortened the deadline. Found by
  -- test, not a pentest.
